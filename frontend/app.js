...
async function updateUI(data) {
    ...
    // Re-initialize DataTable if not already
    ...
    if (weeklyAttendance && Object.keys(weeklyAttendance).length > 0) {
        ...
        // Re-initialize DataTable for weekly report if not already
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        document.getElementById('employees').innerHTML = '<p class="text-red-500">Error loading data.</p>';
        document.getElementById('loading').style.display = 'none';
    }
});
